...
- `FlexibleTextDateDetector` reads a year in a date only in four digits, or beside an
  era ("5 March 44 BC"). A two- or three-digit year cannot be told from a count after a
  date, and was read as one: "5 June 200 attendees" as the year 200, "in June 200 cases"
  as June 200, "on 5 June 20 people came" as the year 20 (0.6.0 read two and four
  digits). It now reads "5 June" there, and "June 20" as a day in the last, as it reads
  "June 20" alone. `FlexibleShortYearDateDetector` deposits exactly the dates it
  withholds, under their own type `date:short-year` ("24 April 350", `("y", 350)`;
  "June 200", `("y", 200)`), generated by `SHORT_YEAR_FAMILY` in `GUARDED_FAMILIES`, so a
  consumer opts in by type. The readers built on the text-date reader follow it:
  `FlexibleDateTimeDetector` no longer reads "June 5, 99 at 3:00 PM", and the month-name
  and bare-hour readers, which step aside for its dates, read what it no longer takes
  ("June" in "in June 200 cases"; the hour "20" in en_GB's "5 June 20 people").
