...
- `flexible_detectors(locale)` assembles the flexible readers of `icukit.recognize` for a
  ...
  symbol of its own; the units CLDR's unit preferences give the world and the locales'
  regions, with the mixed ones ("foot-and-inch") and the runs of CLDR's duration order
  ("hour-and-minute-and-second"). `currencies=` and `units=` choose others, `locales=`
...
- A three-digit year in a text date, as ICU's `y` writes one ("24 April 350"), is read
  under `date:short-year` (see Changed); a one-digit year is not read, since it cannot
  be told from a count after a date. `FlexibleTextDateDetector` reads the abbreviation
  lexicon's weekday forms, with their period and those CLDR does not name ("Sun. 29
  September 1912", "Tues. 3 May 2011", "Thurs., 2 May 2013"), as it reads the lexicon's
  dotted months; the weekday is still checked against the date.

### Changed

- A count after a month is no longer read as a year of one to three digits:
  - `DateDetector` (the default gang's skeleton readers) reads a year from a `y` field
    only in four or more digits, as ICU writes every year from 1000 on; en_US "in June
    200 cases" was `date:yMMMM` with `y` 200, "June 5" and "August 9" `date:yMMMM` with
    `y` 5 and 9, "3/4" `date:yM` with `y` 4, and de_DE "24. April 350" `date:yMMMMd`
    with `y` 350. A `yy` field keeps its two digits ("3/4/24" is 2024).
  - `FlexibleTextDateDetector` reads a year in a date in four digits, or in two or three
    beside an era ("5 March 44 BC"); it read two to four digits (0.6.0: two and four),
    so "5 June 200 attendees" was the year 200, "in June 200 cases" June 200, and "on 5
    June 20 people came" the year 20. It now reads "5 June" there, and "June 20" as a
    day in the last, as it reads "June 20" alone.
  - `FlexibleShortYearDateDetector` deposits exactly the dates `FlexibleTextDateDetector`
    withholds, under their own type `date:short-year` ("24 April 350", `("y", 350)`),
    generated by `SHORT_YEAR_FAMILY` in `GUARDED_FAMILIES`, so a consumer opts in by
    type. Its value is the year as written: "Mar 3, 07" is `("y", 7)`.
  - `FlexibleDateTimeDetector`, built on `FlexibleTextDateDetector`, no longer reads
